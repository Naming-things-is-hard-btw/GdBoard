...
@export var candraw = true:
	set(new):
		...
		if candraw:
			if plugin.brush_manager.use_mouse(): mouse_filter = Control.MOUSE_FILTER_PASS
...
func _process(_delta):
	use_mouse = plugin.brush_manager.use_mouse()
...
func _gui_input(event):
	...
	plugin.brush_manager.canvas_input.emit(event)
	pass
...
func _startdrawing(event):
	...
	is_drawing = true
	plugin.brush_manager.start(final_position, root, old_image, preview)
...
func _updatedrawing(event):
	...
	final_position.x = floor(final_position.x) + 0.5
	final_position.y = floor(final_position.y) + 0.5
	plugin.brush_manager.update(final_position)
	pass

func _canceldrawing():
	if not is_drawing : return
	if not plugin.brush_manager.is_allowed_to_cancel: return
	final_image_vp.render_target_update_mode = SubViewport.UPDATE_DISABLED
	draw_preview_vp.render_target_update_mode = SubViewport.UPDATE_DISABLED
